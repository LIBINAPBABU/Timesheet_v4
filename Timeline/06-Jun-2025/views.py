...
def submission_templete(request):
    ...
    if request.method == 'POST':
        ...
        if mode == 'predate':
            ...
            # handle_assign_milestone(request.user, json.loads(request.POST.get('checkedItems')), present_week_start_date)
        elif mode == 'removemilestone':
            # Attempt to remove a milestone, and return an error if it fails
            # if not handle_remove_milestone(request.user, request.POST, present_week_start_date):
            ...
            # handle_duplicate_milestones(request.user, present_week_start_date)
            handle_duplicate_assign_tasks(request.user, present_week_start_date)
        elif mode == "submit":
            # Submit the user's timesheet for the current week
            # handle_submit_timesheet(request.user, present_week_start_date, current_date)
            handle_submission(request.user, present_week_start_date, current_date)
            ...
            # result = handle_update_timesheet(request)
...
def fetch_quotations(request):
    try:
        ...
        # Get the latest quotations with project details by joining the annotated ids
        # project_list = Quotation.objects.using('erp') \
        #     .filter(Q(id__in=quotation_ids)) \
        #     .select_related('project')  \
        #     .values('quotation_no', 'project__name', 'id', 'sale_type', 'status') \
        #     .order_by('-id')
...
def fetch_projects(request,weekStartDate):
    try:
        ...
        # selectedquotations = list(AssignedMilestone.objects.values('quotation','project','projectName').filter(user=request.user.id,end_date__gte = weekStartDate,start_date__lte = weekEndDate).distinct())
        selectedquotations = list(AssignedTask.objects.values('quotation','project','projectName','customerName','customprojectName','customerCode','systemName')
...
def handle_assign_project(user, quotations, weekStartDate):
    try:
        # Clear existing quotations associated with the user
        logger.info(f"Clearing existing quotations for user: {user}")
        AssignedQuotation.objects.filter(user=user).delete()
        ...
        # Prepare a list to hold the AssignedQuotation objects
        assigned_quotations = []

        # Iterate over the quotations and create instances
        for quotation in quotations:
            assigned_quotations.append(AssignedQuotation(quotation=quotation, user=user))

        # Bulk create the objects in one query
        try:
            logger.info(f"Creating {len(assigned_quotations)} assigned quotations for user: {user}")
            # AssignedQuotation.objects.bulk_create(assigned_quotations)
        except IntegrityError as e:
            logger.error(f"Integrity error while bulk creating assigned quotations: {e}")
            raise DatabaseError("Failed to create assigned quotations due to integrity issues.") from e
        except DatabaseError as e:
            logger.error(f"Database error while bulk creating assigned quotations: {e}")
            raise DatabaseError("Failed to create assigned quotations.") from e
        ...
        try:
            ...
            quotationDetails1 = list(Quotation.objects.using('erp').annotate(
        ...
        # handle_assign_milestone(user, quotationDetails, weekStartDate)
        handle_assign_tasks(user, quotationDetails1, weekStartDate)
    except Exception as e:
        logger.error(f"An unexpected error occurred while assigning project: {e}")
        raise e  # Reraise the error after logging it
    
def handle_assign_milestone(user, quotationDetails, start_date):
    try:
        ...
        assignments = []
        # Fetch existing assigned tasks in one go
        existing_tasks = AssignedMilestone.objects.filter(
                quotation__in=[item['quotation_'] for item in quotationDetails],
                milestone__in=[item['milestone'] for item in quotationDetails],
                milestoneName__in=[item['milestoneName'] for item in quotationDetails],
                task__in=[item['task'] for item in quotationDetails],
                taskName__in=[item['taskName'] for item in quotationDetails],
                user=user.id
            ).values('quotation',
                    'project', 
                    'projectName',
        ...
        assigned_tasks_map = {
            (task['quotation'], task['project'], task['projectName'], task['milestone'], task['milestoneName'], task['task'], task['taskName']): task
            for task in existing_tasks
        }
        # Prepare updates and new assignments
        updates = []
        for item in quotationDetails:
            key = (item['quotation_'],item['project'], item['projectName'],item['milestone'], item['milestoneName'], item['task'], item['taskName'])
            assigned_task = assigned_tasks_map.get(key)
    
            if assigned_task:
                if assigned_task['start_date'] > start_date.date():
                    updates.append((item['quotation_'],item['project'], item['projectName'],item['milestone'], item['milestoneName'], item['task'], item['taskName'], start_date, None))
                else:
                    updates.append((item['quotation_'],item['project'], item['projectName'],item['milestone'], item['milestoneName'], item['task'], item['taskName'], None, start_date + timedelta(days=6)))
            else:
                nullMilestoneTask = AssignedMilestone.objects.filter(quotation=item['quotation_'],milestone=None,milestoneName=None,task=None,taskName=None).filter(user = user).first()
                if nullMilestoneTask:
                    nullMilestoneTask.project = item['project']
                    nullMilestoneTask.projectName = item['projectName']
                    ...
                    assignments.append(AssignedMilestone(
        ...
        if assignments:
            logger.info(f"Bulk creating {len(assignments)} new milestone assignments for user: {user.id}")
            try:
                AssignedMilestone.objects.bulk_create(assignments)
                logger.info(f"Successfully created new milestone assignments for user: {user.id}")
            except IntegrityError as e:
                logger.error(f"Integrity error while bulk creating milestone assignments: {e}")
                raise DatabaseError("Failed to create new milestone assignments due to integrity issues.") from e
            except DatabaseError as e:
                logger.error(f"Database error while bulk creating milestone assignments: {e}")
                raise DatabaseError("Failed to create new milestone assignments.") from e

        # Apply updates if any
        if updates:
            logger.info(f"Applying {len(updates)} updates to existing milestone assignments for user: {user.id}")
            try:
                for quotation_no,project, projectName,milestone, milestoneName, task, taskName, start, end in updates:
                    update_data = {}
                    if start is not None:
                        update_data['start_date'] = start
                    if end is not None:
                        update_data['end_date'] = end
                    AssignedMilestone.objects.filter(
                ...
                logger.error(f"Integrity error while updating milestone assignments: {e}")
                raise DatabaseError("Failed to update milestone assignments due to integrity issues.") from e
            except DatabaseError as e:
                logger.error(f"Database error while updating milestone assignments: {e}")
                raise DatabaseError("Failed to update milestone assignments.") from e
    except Exception as e:
        logger.error(f"An unexpected error occurred while handling milestone assignments for user: {user.id}: {e}")
        raise e  # Reraise the error after logging it
...
def handle_assign_tasks(user, quotationDetails, start_date):
    try:
        ...
        existing_tasks = AssignedTask.objects.filter(
                quotation__in=[item['quotation_'] for item in quotationDetails],
                customerName__in=[item['customerName'] for item in quotationDetails],
                customprojectName__in=[item['customprojectName'] for item in quotationDetails],
                systemName__in=[item['systemName'] for item in quotationDetails],
                customerCode__in=[item['customerCode'] for item in quotationDetails],
        ...
        for item in quotationDetails:
            ...
            if assigned_task:
                ...
                if nullMilestoneTask:
                    # nullMilestoneTask.customerName =item['customerName'], 
                    # nullMilestoneTask.customprojectName =item['customprojectName'], 
                    # nullMilestoneTask.systemName = item['systemName'], 
                    # nullMilestoneTask.customerCode = item['customerCode'],
                    # nullMilestoneTask.project = item['project'],
                    # nullMilestoneTask.projectName = item['projectName']
                    nullMilestoneTask.milestone = item['milestone']
                    nullMilestoneTask.milestoneName = item['milestoneName']
        ...
        # Bulk create new assignments
        if assignments:
            logger.info(f"Bulk creating {len(assignments)} new milestone assignments for user: {user.first_name}")
            try:
                AssignedTask.objects.bulk_create(assignments)
                logger.info(f"Successfully created new milestone assignments for user: {user.first_name}")
            except IntegrityError as e:
                logger.error(f"Integrity error while bulk creating milestone assignments: {e}")
                raise DatabaseError("Failed to create new milestone assignments due to integrity issues.") from e
            except DatabaseError as e:
        ...
        # Apply updates if any
        if updates:
            logger.info(f"Applying {len(updates)} updates to existing milestone assignments for user: {user.first_name}")
            try:
        ...
        logger.error(f"An unexpected error occurred while handling milestone assignments for user: {user.first_name}: {e}")
        raise e  # Reraise the error after logging it
...
def prepare_context_task_data(user, start_date, end_date, current_date):
    #     # ......................timesheet table data
        ...
        try:
            ...
            logger.info(f"Fetched {len(timesheet_status)} timesheet statuses for user {user.id} for week {start_date.strftime('%V')}")
        except TimesheetStatus.DoesNotExist:
            timesheet_status = None  # or set default status like 'Not Found'
        except DatabaseError as e:
...
def handle_Timesheet_data(data, projectHoursDict, startDate, submissionData):
    ...
    for item in data:
        ...
        milestone_data = {
            ...
            # "workedHours": append_default_actual_worked_hours({}, startDate),
            "tasks": [task_data]
        }
...
# old update timesheet ..........................
def handle_update_timesheet(request):
    try:
        # Parse date, task ID, hours, and project ID from request
        date = datetime.strptime(request.POST.get('date'), "%d-%m-%Y").date()
        quotation_id = request.POST.get('quotation_id')
        project_id =  request.POST.get('project_id') if request.POST.get('project_id')!="null" else None
        project_name = request.POST.get('project_name') if request.POST.get('project_name')!="null" else None
        milestone_id = request.POST.get('milestone_id') if request.POST.get('milestone_id')!="null" else None
        milestone_name = request.POST.get('milestone_name')
        task_id = request.POST.get('task_id') if request.POST.get('task_id')!="null" else None
        task_name = request.POST.get('task_name')
        hours = request.POST.get('hours')
        userid = request.POST.get('empl_id', request.user.id)
        # Fetch existing submission data for the given task and date
        submission_data = TimesheetSubmission.objects.filter(
                                                    uid=userid,
        ...
        # Calculate total hours for the day excluding current task
        hrs_per_day = TimesheetSubmission.objects.filter(
                uid=userid,
                date=date
            ).aggregate(sum_colum=Sum('hours'))
        ...
        # Calculate total duration including existing hours
        duration_ = duration + (hrs_per_day['sum_colum'] or 0)
        if submission_data:
            duration_ -= submission_data.hours
        # Check if total duration exceeds limit (14 hours)
        if duration_ <= 50400:
            if not submission_data:
                # Create new timesheet entry if none exists
                submission_model = TimesheetSubmission(
                    uid_id=userid,
                    date=date,
                    hours=duration,
                ...
                logger.info(f"Creating new timesheet entry for {userid} on {date} for task {task_name} with {duration} hours")
            else:
                # Update existing timesheet entry
                submission_model = submission_data
                if duration != 0.0:
            ...
            if duration != 0.0:
                logger.info(f"Updating timesheet entry for {userid} on {date} for task {task_name} with {duration} hours")
                submission_model.save()
            return {"status": "valid"}
        else:
            logger.info(f"Deleting timesheet entry for {userid} on {date} for task {task_name} as hours are zero")
            return {"status": "invalid"}
    except DatabaseError as db_error:
        # Log database errors
        logger.error("Database error while fetching quotations for user %s: %s", request.user.id, db_error)
...
def handle_duplicate_milestones(user, start_date):
    try:
        AssignedMilestone.objects.filter(
            Q(end_date__range=[start_date, start_date + timedelta(days=6)]) &
            Q(user=user.id)
...
def handle_remove_milestone(user, post_data, start_date):
    try:
        assignedIdList = json.loads(post_data.get('assignedIdList'))
        timesheetData = AssignedMilestone.objects.values('id','quotation','milestone','milestoneName','task','taskName').filter(id__in=assignedIdList)
        if get_submission_Status(timesheetData, start_date, user):
            assigned_milestones = AssignedMilestone.objects.filter(pk__in=assignedIdList)
            for assigned_milestone in assigned_milestones:
                if assigned_milestone.start_date < start_date.date():
                    assigned_milestone.end_date = start_date - timedelta(days=1)
                    assigned_milestone.save()
                else:
...
def handle_submission(user, start_date, current_date):
...
def get_submission_Status(data,startDate,user):
    try:
        for item in data:
            try:
                if TimesheetSubmission.objects.filter(Q(date__gte = startDate)&
                                                    Q(uid = user.id)&
                                                    Q(quotation = item['quotation'])&
                                                    Q(milestone = item['milestone'])&
                                                    Q(milestoneName = item['milestoneName'])&
                                                    Q(task = item['task'])&
                                                    Q(taskName = item['taskName'])):
                    return False
            except DatabaseError as db_error:
...
# Old timesheet submission
def handle_submit_timesheet(user, start_date, current_date):
    timesheet_entries = TimesheetSubmission.objects.filter(
        Q(uid=user.id),
        Q(date__range=[start_date, start_date + timedelta(days=6)]),
        Q(Q(timesheet_status="Rejected")|Q(timesheet_status= None))
    )
    for entry in timesheet_entries:
        entryObject = TimesheetSubmission.objects.get(pk=entry.id)
        entryObject.timesheet_status = "Submitted"
        entryObject.save()
...
def handle_rejection_reasons(user, start_date):
    rejected_reasons = TimesheetSubmission.objects.filter(
        uid=user.id,
...
def unlock_timesheet(request):
    ...
    try:
        ...
        try:
            week_start_date = datetime.strptime(from_date_str, '%d-%b-%Y').date()
        except ValueError:
            return JsonResponse({'success': False, 'message': 'Invalid date format.'}, status=400)

        # Calculate day difference
        # today = datetime.today().date()
        # day_difference = (today - week_start_date).days

        week_start_date = datetime.strptime(from_date_str, '%d-%b-%Y')
        day_difference = (datetime.today().date() - week_start_date.date()).days

        # Validation for day difference (1 to 13 inclusive)
        if not (0 < day_difference <= 13):
        ...
        if existing_timesheet:
            ...
            TimesheetStatus.objects.create(
                ...
                weeknumber=week_start_date.strftime("%V"),
                submission_status=(week_start_date + timedelta(days=6)).date() >= datetime.today().date(),
                action_status=getSubmissionStatus(week_start_date),
...
def emailsend(request):
    ...
    data =users.objects.values('first_name','email').annotate(
                                            ...
                                            ).exclude(Q(id__in = excludeList)).filter(Q(is_active=True) & ~Q(role__in =["Admin","HR","Consultant"]))
