...
def submission(request):
    try:
            assigned_task = AssignedTask.objects.filter(
                        ...
                        end_date__gte='2025-08-04',
                        start_date__lte='2025-08-10'
                    ).values(
                        'quotation',
                        'customerName',
                        'customprojectName',
            ...
            logger.error(f"Error fetching assigned milestones for user: {e}")
            raise
    pageData = PageGroup.objects.filter(jobtitle_id = request.user.jobtitle_id , page__module__name = "Timesheet").values_list('page__name',flat=True)
    return render(request,'submission.html',{"module":"timesheet","pageData":list(pageData)})
...
def submission_templete(request):
    ...
    hoursRestrictionList = cache.get("hoursRestrictionList")
    if request.user.jobtitle_id in hoursRestrictionList:
        hoursRestriction = True
    else:
        hoursRestriction = False
    context['hoursRestriction'] = hoursRestriction
...
def fetch_quotations(request):
    try:
        ...
        quotation_ids = Quotation.objects.using('erp') \
            ...
            .exclude(status__in = ["Closed","Lost"]).values_list('latest_id', flat=True)
...
def prepare_context_task_data(user, start_date, end_date, current_date):
    ...
    #     # ......................timesheet table data
        ...
        try:
            ...
            raise
        try:
            Assigners =list(users.objects.values('first_name','id').filter(Q(is_active=True) & Q(jobtitle_id__in = excludejobTitleList)).exclude(id= user.id).order_by('first_name'))
        except Exception as e:
...
def handle_Timesheet_data(data, projectHoursDict, startDate, submissionData): 
    ...
    for item in data: 
        ...
        filteredData =  submissionData.filter(quotation=quotationId)
        milestone_worked_hours = calculate_milestone_worked_hour(filteredData)
        task_worked_hours = calculate_task_worked_hours(filteredData)
...
def unlock_timesheet(request):
    ...
    try:
        ...
        unlockAllowedWeeks = cache.get("unlockAllowedWeeks")
        unlockDayLimit = (int(unlockAllowedWeeks)*7)-1

        if not (0 < day_difference <= unlockDayLimit):
        ...
        if existing_timesheet:
            if existing_timesheet.unlock_status in ["Accepted", "locked"]:
...
def allTimesheetLog(request):
    ...
    pageData = PageGroup.objects.filter(jobtitle_id = request.user.jobtitle_id , page__module__name = "Timesheet").values_list('page__name',flat=True)
    return render(request,'approval.html',{"module":"timesheet","pageData":list(pageData)})  
...
def get_summary_template(request):
    ...
    context,unlockResqued = prepare_context_approval_data(request.user, week_start_date, week_end_date, current_date, excludejobTitleList)
...
def prepare_context_approval_data(user, week_start_date, week_end_date, current_date,excludejobTitleList):
    # Base query for fetching active users who joined before or on the week start date, excluding certain roles
    tabledata = users.objects.exclude(jobtitle_id__in=excludejobTitleList) \
...
def emailsend(request):
    ...
    queryFilterList = {}
    weeklyMailList = cache.get("weeklyEmailList")
    excludejobTitleList = cache.get("excludeList")
    if weekday == 4:
        ...
        if (8 <= day and day < 15) or (22 <= day and day < 29):
            ...
            queryFilterList['jobtitle_id__in'] = weeklyMailList
            if current_hour != 16:
                queryFilterList['reporting_to__jobtitle_id__in'] = weeklyMailList
        else:
            return ''
    if weekday == 5:
        day = currentDate.day
    ...
    weeklySummaryMailList = cache.get("notSubmittedSummaryMailList")
    weeklySummaryMailDetails = users.objects.values('first_name','email').filter(id__in=weeklySummaryMailList)
    ...
    data = users.objects.values('first_name','email').annotate(
                                            ...
                                            ).exclude(Q(id__in = excludeList) | Q(jobtitle_id__in = excludejobTitleList) | Q(is_active = False)).filter(**queryFilterList)
    print(data)
...
def dailyRemainderEmail(request):
    ...
    try:
        ...
        yesterdayNotFilledUsers = users.objects.values('first_name','email').annotate(
                                                _reportingto= F('reporting_to__email')
                                                ).exclude(Q(id__in = yesterdayList)).filter(Q(is_active=True) & ~Q(jobtitle_id__in = excludejobTitleList) & Q(jobtitle_id__in = dailyEmailList))
    except Exception as e:
...
def weeklyApprovalMail(request):
    ...
    notApprovedUsers = list(
        ...
        .filter(reporting_to__jobtitle_id__in = weeklyMailList)
        .annotate(
            _reportingto=F('reporting_to__email'),
