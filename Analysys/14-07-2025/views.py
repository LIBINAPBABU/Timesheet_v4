...
def getNonSubmittData(request):
    if request.method == 'POST':
        ...
        week_end_date = datetime.fromisocalendar(selectedWeekYear, int(weekEnd), 1)
        ...
        if statusType == 'submission_status':
           ...
           # Get the full date range of the selected weeks
            ...
            for i in range(weekStart, weekEnd + 1):
                ...
                ).values('approvedBy__first_name', 'approvedBy').annotate(
                    submitted_count=Count('approvedBy', filter=Q(status='Submitted', approved_status=0)),
                ...
                for item in weekly_data:
                    ...
                    elif item['ontimeApproval_count']:
                        approvalList[name]['ontimeApproved_count'] += 1
                    elif item['delayedApproval_count']:
