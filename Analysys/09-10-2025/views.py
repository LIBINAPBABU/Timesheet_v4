...
def getConsolidatedReportData(request):
    ...
    if request.method == 'POST':
        ...
        if statusType == 'submission_status':
            ...
            for i in range(weekStart, weekEnd + 1):
                if current_weekyear == selectedWeekYear and current_week == i:
                    continue
                week_start_date_ = datetime.fromisocalendar(selectedWeekYear, int(i), 1)
                ...
                for item in weekly_data:
                    approvedId = item['approvedBy']
                    if item['submitted_count']:
                        approvalList[approvedId]['notApproved_count'] += 1
                    elif item['delayedApproval_count']:
                        approvalList[approvedId]['delayedApproved_count'] += 1
                    elif item['ontimeApproval_count']:
                        approvalList[approvedId]['ontimeApproved_count'] += 1

                    approvalList[approvedId]['name'] = item['approvedBy__first_name']
                    approvalList[approvedId]['joinedWeek'] = item['joinedWeek']
                    approvalList[approvedId]['joinedYear'] = item['joinedYear']
                    approvalList[approvedId]['resignedWeek'] = item['resignedWeek']
                    approvalList[approvedId]['resignedYear'] = item['resignedYear']
