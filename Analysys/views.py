...
from Employee.models import users,PageGroup
from django.http import HttpResponse, JsonResponse
from Timeline.views import secondToHours
from Timeline.models import Quotation,TimesheetStatus,Submission,Customer,AssignedTask
...
def weeklyTimesheetStatus(request):
    pageData = PageGroup.objects.filter(jobtitle_id = request.user.jobtitle_id , page__module__name = "Analysis").values_list('page__name',flat=True)
    return render(request,'weeklyTimesheetStatus.html',{"module":"analysys","pageData":list(pageData)})
...
#     selectedEmployee = ''
#     print(selectedYear)
#     print(selectedWeekStart,selectedWeekEnd)
...
#                                                 weekyear=Coalesce(F('timesheetstatus__weekyear'),Value(selectedYear))
#                                             )\
#                                             .values('first_name', 'reporting_to', 'id', 'joinedWeek', 'joinedYear',
#                                                     'submission_status', 'action_status', 'timesheet_status' , 'weeknumber', 'weekyear')
...
#             base_filter['reporting_to'] = request.user.id
#     else:
#         base_filter['id'] = selectedEmployee
#     employees = employees.filter(**base_filter)
#     print(employees)

# # ------------------------------------------------------------------------------------------------
#     weekStartDate = datetime.fromisocalendar(selectedYear, int(selectedWeekStart), 1)
#     weekEndDate = datetime.fromisocalendar(selectedYear, int(selectedWeekEnd), 7)

#     #Get the approval status of the employee for the week
#     timesheetTaskStatus = Submission.objects.filter(
#         date__range =[weekStartDate, weekEndDate]).values('status','approved_status')
# # ------------------------------------------------------------------------------------------------
...
#                                                   "approval_status": "notsubmitted" if item['weeknumber'] is None else  getApprovalStatus(timesheetTaskStatus,item['id'],item['weeknumber'],item['weekyear']),
#                                                   #"approval_status": "notsubmitted" if item['weeknumber'] is None else  getApprovalStatus(item['id'],item['weeknumber'],item['weekyear']),
...
# def getApprovalStatus(timesheetTaskStatus,user,weeknumber,year):
#     # get start date from weeknumber and year
#     start_Date = datetime.fromisocalendar(year, int(weeknumber), 1)
    
#     #Get the approval status of the employee for the week
#     timesheetTaskStatus = timesheetTaskStatus.filter(
#         assignId__assignTo=user,date__range =[start_Date, start_Date + timedelta(days=6)])
#     print(timesheetTaskStatus)
#     # timesheetTaskStatus = Submission.objects.filter(
#     #     assignId__assignTo=user,date__range =[start_Date, start_Date + timedelta(days=6)]).values('status','approved_status')

#     taskStatus = list(timesheetTaskStatus.values_list('status', flat=True).distinct())
#     approvedStatus = list(timesheetTaskStatus.values_list('approved_status', flat=True).distinct())

#     if taskStatus:
#         if 'Rejected' in taskStatus or None in taskStatus:
#             return 'notsubmitted'

#         if len(taskStatus) == 1:
#             if 'Accepted' in taskStatus:
#                 if 1 in approvedStatus:
#                     return 'ontime'
#                 else:
...
def getWeeklyTimesheetStatus(request):
    ...
    # employees = users.objects.prefetch_related('timesheetstatus').filter(is_active=True) \
    #     .exclude(role__in=['Consultant', 'Admin', 'HR', 'CEO']) \
    #     .annotate(
    #         joinedWeek=ExtractWeek('date_joined'),
    #         joinedYear=ExtractYear('date_joined'),
    ...
    #         weekyear=Coalesce(F('timesheetstatus__weekyear'), Value(selectedYear))
    #     ).values(
    #         'first_name', 'reporting_to', 'id', 'joinedWeek', 'joinedYear',
    #         'submission_status', 'action_status', 'timesheet_status',
    #         'weeknumber', 'weekyear'
    #     )
    excludejobTitleList = cache.get("excludeList")
    employees = users.objects.annotate(
        ...
        # ).exclude(role__in=['Consultant', 'Admin', 'HR', 'CEO'])\
        ).exclude(jobtitle_id__in=excludejobTitleList)\
    ...
    elif request.user.jobtitle_id not in excludejobTitleList:
...
def getConsolidatedReportData(request):
    ...
    if request.method == 'POST':
        ...
        if statusType == 'submission_status':
            # .exclude(role__in=['Consultant', 'Admin', 'HR', 'CEO'])
            data = users.objects.exclude(jobtitle_id__in=excludejobTitleList).annotate(
                joinedWeek=ExtractWeek('date_joined'),
                joinedYear=ExtractYear('date_joined'),
                resignedWeek=ExtractWeek('resigned_date'),
                resignedYear=ExtractYear('resigned_date'),
            ...
            # data = TimesheetStatus.objects.filter(
            #     timesheet_status__in=['Submitted', 'Rejected', 'Accepted'],
            #     weekyear=selectedWeekYear,
            #     weeknumber__range=(weekStart,weekEnd)
            # ).values('uid__first_name').annotate(
            #     joinedWeek=ExtractWeek('uid__date_joined'),
            #     joinedYear=ExtractYear('uid__date_joined'),
            #     resignedWeek=ExtractWeek('uid__resigned_date'),
            #     resignedYear=ExtractYear('uid__resigned_date'),
            #     submittedCount=Count('uid'),
            #     ontimecount=Count('uid', filter=Q(submission_status=True)),
           ...
           # Get the full date range of the selected weeks
            week_start_date = datetime.fromisocalendar(selectedWeekYear, int(weekStart), 1)
            week_end_date = datetime.fromisocalendar(selectedWeekYear, int(weekEnd), 7)
            # .exclude(approvedBy__role__in=['Consultant', 'Admin', 'HR', 'CEO'])
            ...
            approval_status = Submission.objects.filter(
                ...
                ).exclude(approvedBy__jobtitle_id__in=excludejobTitleList).annotate(
                    joinedWeek = ExtractWeek('approvedBy__date_joined'),
                    joinedYear = ExtractYear('approvedBy__date_joined'),
...
def timeBudgetAnalysis(request):
    ...
    return render(request,'time_budget_analysis.html',{"module":"analysys",
                                                       'data':list(data),"pageData":list(pageData)})
...
def timesheet_analysis(request):
    pageData = PageGroup.objects.filter(jobtitle_id = request.user.jobtitle_id , page__module__name = "Analysis").values_list('page__name',flat=True)
    return render(request,'timesheet_analysis.html',{"module":"analysys","pageData":list(pageData)}) 
