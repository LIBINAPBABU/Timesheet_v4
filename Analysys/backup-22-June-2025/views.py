...
def getWeeklyTimesheetStatus(request):
    ...
    employees = users.objects.prefetch_related('timesheetstatus')\
                                            ...
                                            .annotate(
                                                ...
                                                weekyear=Coalesce(F('timesheetstatus__weekyear'),Value(current_weekyear))
                                            )\
...
def getApprovalStatus(user,weeknumber,year):
    ...
    approval_status = Submission.objects.filter(
        ...
        submitted_count=Count('assignId', filter=Q(status='Submitted')),
        null_count=Count('assignId', filter=Q(status__isnull=True)),
    )
    ...
    if approval_status['approved_true'] and approval_status['approved_false']:
        ...
        if approval_status['null_count']:
            timesheetstatus = 'notsubmitted'
        elif approval_status['submitted_count'] == approval_status['total']:
            timesheetstatus = 'approvalpending'
        elif approval_status['submitted_count'] != 0:
            timesheetstatus = 'missed'
        else:
            timesheetstatus = 'delayedapproval'
    else:
...
def getNonSubmittData(request):
    if request.method == 'POST':
        ...
        approvalList = {}
        week_start_date = datetime.strptime(f'{selectedWeekYear} {int(weekStart):02d} 1', '%Y %W %w') - timedelta(days=2)
        week_end_date = datetime.strptime(f'{selectedWeekYear} {int(weekEnd):02d} 1', '%Y %W %w') - timedelta(days=2)
        ...
        for i in range(weekStart,weekEnd+1):
            week_start_date_ = datetime.strptime(f'{selectedWeekYear} {int(i):02d} 1', '%Y %W %w') - timedelta(days=2)
            data = list(approval_status.filter(date__range=[week_start_date_, week_start_date_+ timedelta(days=6)]).annotate(
                total=Count('approvedBy'),
                approved_true=Count('approvedBy', filter=Q(approved_status=True)),
                approved_false=Count('approvedBy', filter=Q(approved_status=False)),
                submitted_count=Count('approvedBy', filter=Q(status='Submitted')),
                null_count=Count('approvedBy', filter=Q(status__isnull=True)),
            ).values('approvedBy__first_name','approvedBy','approved_status','status','total','approved_true','approved_false','submitted_count','null_count').distinct())
            approvalList[i] = data

        for week_number, approvals in approvalList.items():
            week_start_date_ = datetime.strptime(f'{selectedWeekYear} {int(week_number):02d} 1', '%Y %W %w') - timedelta(days=2)
            for item in approvals:
                if item['approved_true'] and item['approved_false']:
                    item['timesheetstatus'] = 'approval missed'
                elif item['approved_true'] == item['total'] and item['total'] != 0:
                    item['timesheetstatus'] = 'ontime'
                elif item['approved_false'] == item['total'] and item['total'] != 0:
                    if item['null_count']:
                        item['timesheetstatus'] = 'not submitted'
                    elif item['submitted_count'] == item['total']:
                        item['timesheetstatus'] = 'approval pending'
                    elif item['submitted_count'] != 0:
                        item['timesheetstatus'] = 'approval missed'
                    else:
                        item['timesheetstatus'] = 'delayed approval'
                else:
                    item['timesheetstatus'] = 'unknown'
        ...
        # get the week start date from week year and week number
        # if weekYear and weekNumbersList:
        ...
        #                     Q(weekyear=weekYear,weeknumber__in=weekNumbersList) ,
        ...
        #             employees = users.objects.prefetch_related('timesheetstatus').filter(
        #                 Q(is_active=True),
        #                 ~Q(id__in = submitted_employees),
        #             ).exclude(role__in=['Consultant', 'Admin', 'HR', 'CEO']).annotate(weeknumber_=F('timesheetstatus__weeknumber')).values('first_name','weeknumber_').distinct()
        #             print(employees)
        #         except ValueError as e:
        #             print("Error parsing date:", e)
        #             return JsonResponse({"error": "Invalid week year or week number."}, status=400)
        ...
        #         try:
        #         # Make sure weekNumber is two digits and minus 2 days to get the start date
        #             week_start_date = datetime.strptime(f'{weekYear} {int(weekStart):02d} 1', '%Y %W %w') - timedelta(days=2)
...
def timeBudgetAnalysis(request):
    ...
    submissions = (
        ...
        .annotate(
            emp_count=Count('assignId', distinct=True),  # count unique employees
