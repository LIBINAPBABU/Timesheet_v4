...
from Timeline.models import QuotationCost #importing Timeline model from Timeline application
...
def suggestion(request):
    if request.method == "POST":
        ...
        if mode:  # check if 'mode' is in POST data
            ...
            if id:
                ...
                suggestionObject.remarks = remarks
    ...
    data = list(Suggestion.objects.values('id','uid','uid__first_name', 'remarks', 'suggestion_status','created_date'))
    return render(request, 'suggestion.html', {'data': list(data), 'module': 'document'})
